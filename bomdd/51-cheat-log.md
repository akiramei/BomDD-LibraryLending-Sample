...
### CHEAT-F01-H002 [observer_comparator_defect] PowerShell 変数名の大文字小文字非区別による比較無効化
- side: **harness** / 発生段: ⑥合否(v1 から潜在、**rev2 のセルフテスト必須化が凍結前に捕捉**)
- 内容: `Test-ErrorShape` 内のローカル `$code` がパラメータ `$Code` を上書き(PowerShell は変数名の大小を区別しない)し、「error.code が存在しさえすれば期待コードと一致扱い」になっていた。
- 影響の再検証: v1 採点ログには実コードが印字されており全件期待値と一致。さらに修正後の比較器で v1 成果物を再採点し **opus/sonnet とも S01–S20 全PASS を再確認**(result-*-revalidation.json)。**判定結果への影響なし**。
- 補正: ローカル変数名を `$actualCode` に変更+`-ceq`(大小区別)へ強化。
- 意義: 治具セルフテスト(rev2 で必須化した工程)が**初回実施で実バグを捕捉**。「治具は未検証のまま初回採点に臨む」リスクの直接実証2例目。
### H-003(改善・ずるではない)
