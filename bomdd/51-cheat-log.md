...
### CHEAT-F015-H001 [oracle_scenario_defect] M03 のシナリオ日付が判定順序と干渉(較正が凍結前に捕捉)
- side: **harness** / 発生段: ③BOM設計(rev3 オラクル設計)
- 内容: マイグレーションオラクル M03 の新規貸出日(6/20)が fixture 既存貸出の期限(5/15・5/16)より後で、仕様の判定順序3(延滞ブロック)が上限判定(順序4)より先に発火 → 上限の検査にならないシナリオだった。
- 捕捉: **negative control 較正(v0.2 個体への実行)が凍結前に検出**。日付を 5/10 へ修正後、較正は想定プロファイル(migration 4/4・fixed v3 23/25=S24/S25 のみ FAIL)に一致。
- 意義: 治具セルフテスト(静的)に加え、**較正(動的 negative control)**が第3の検査器ずる検出装置として機能。フォワード治具リスク対策の2段目。
