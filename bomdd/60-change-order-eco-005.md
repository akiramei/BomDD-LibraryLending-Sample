...
| 測定 | factory-eco5-01-sonnet(fresh・採用= v0.6) |
|---|---|
| 自己受入 | 37/37(unit 31+L1 スモーク 6)・cheat-report **0 件** |
| diff 監査(63・設計者独立検証) | **Store.cs(14 行)+Library.Api.csproj(3 行)のみ** = 影響分析どおり・不要改変 0・旧部品参照 grep 0 件 |
| 回帰 S01–S28 | **28/28** = regression 0 |
| 効力 E01–E04 | **4/4** |
| 移行回帰 M01–M04(最リスク= **旧部品が書いた実 DB** を新部品で開いて移行) | **4/4** = 「永続化(データ互換性)」沈黙次元の初検査に合格 |
| 変更受入 V01(脆弱 0 件・供給網離脱の確認) | **1/1**(SQLitePCLRaw 依存 0) |
| 影響なし予測 | **全的中**(under/over-inclusion 0 — Core/test/Program.cs diff ゼロ) |

- **昇格連鎖の成立(本 loop の主測定)**: 検出(DEG-002)→ 逆引き(1/3 SB)→ 交換候補 →
  **scratch 赤(build 6 エラー)** → 昇格裁定「**再製造必要**」(語彙第4項の初適用)→ K/M 層のみ改訂
  (REQ/仕様/E-BOM/オラクル**全て無改訂** — 挙動契約が部品に非依存であることの実証)→
  fresh 工場の部分再製造(1 unit)→ 全再認証、が一巡した。
- **昇格の決定点は scratch 事前検証に前倒しされた**: 「適用後に回帰が赤くなる」事象は発生しない —
  決定的な非互換は forward-03 で工程化した scratch 検証が先取りする。適用後の回帰赤が残るのは
  scratch で再現できない非互換(環境依存・非決定的・実データ規模依存)に限られる(method 還元)。
- **K-BOM が適応の裁量を吸収**: K-SQLITE-001 v3 に型対応・パラメータ形式・PRAGMA 可用性を明記した
  結果、工場の cheat-report は **0 件**(forward 系列で初)— 部品交換の適応は知識パックで裁量ゼロにできる。
- 治具側の副観測(H 系列): V01 の健全性照合が日本語コンソールの符号化事故を**判定不能 fail として
  正しく検出**(偽緑防止ガードの実証)→ 恒久対処として治具内で DOTNET_CLI_UI_LANGUAGE=en を固定。
- 採用: factory-eco5-01-sonnet を As-Maintained **v0.6** とする(tag `v0.6-forward-04`。
  watch CI の AS_MAINTAINED も更新)。
