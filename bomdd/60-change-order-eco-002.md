...
## 6. 記録(2026-07-04 実施)
| 測定 | factory-eco2-01(opus・採用) | factory-eco2-02(sonnet・対照) |
|---|---|---|
| 自己受入 | 37/37(unit 31+L1 スモーク 6。stop/report 発動なし) | 37/37(同) |
| 回帰 S01–S25 | 25/25 = **regression 0** | 25/25 = **regression 0** |
| 変更受入 S26–S28 | 3/3 = **change miss 0** | 3/3 = **change miss 0** |
| 効力 E01–E04 | 4/4 = **効力違反 0** | 4/4 = **効力違反 0** |
| 移行回帰 M01–M04 | 4/4 | 4/4 |
| 不要改変 diff | **0**(LendingDomain.cs+Program.cs 配線1行[事前申告]+test-only) | **0**(同・Program.cs の diff は opus と blob 同一) |

- **影響なし予測の成績(本 ECO の主測定)**: **under-inclusion 1 / over-inclusion 0**。
  - 反証の構造: §2 の「dueDateText は Core の判定関数が返す=配線変更も不要」は**実物で不成立** —
    As-Maintained 個体は期限計算を Program.cs の別呼び出しサイト(decide コールバック内の直接呼び出し)に
    置いており、`ctx.MemberType` を通す配線1行が不可避だった。**2 fresh 工場が事前申告つきで同一の1行に
    独立収束(git blob 同一)**=この diff は工場の逸脱でなく影響分析の取りこぼし。
  - 粒度の分解: **E-BOM 粒度の予測(E-DUE-FINE-001 のみ=1/12)は的中**(配線1行も同品目の実装)。
    外れたのは品目→unit の写像 — 実装が1品目を unit 横断(Core の規則+Api の配線)に分布させていた。
    教訓: 影響分析は BOM 上のトレース逆引きに加えて **As-Maintained 実物の呼び出しサイト点検**を要する
    (「実物が正」の影響分析版。method 還元候補)。
- **効力(effectivity)の初検証成立**: 既存 premium 貸出の dueDateUtc 不変(E02)・確定値基準の料金(E04)・
  新規貸出のみ新規則(E03)。両工場とも FMEA-006(期間再計算による遡及)を作り込まなかった
  (`Fine`/`IsMemberOverdue` が確定済み DateOnly を引数に取る既存構造が遡及を構造的に防いだ)。
- 絞り込みの実測: 影響 E 品目 1/12・src diff 3ファイル(うち実質規則変更は Core 1ファイル)。
  ECO-001(E 4/12・4ファイル)より局所的な変更で、予測の書き方(反証可能な「diff ゼロ」宣言)が
  under-inclusion を**1行単位で可視化**した — 絞り込み予測は外れ方まで測定できる形で書くのが正しい。
- 採用個体: **factory-eco2-01(opus)**(v0.2=factory-04 からの系譜継続)。result tag: `v0.4-forward-02`。
- 副産物(既存債務・ECO-002 スコープ外): 受入ハーネスの構成不一致 — L1 スモークが `dotnet run --no-build`
  (既定 Debug)で API を起動するため、`-c Release` ビルドのみだと偽赤になる(opus が検出・申告。
  Debug ビルド併走で全緑)。ハーネス改修は別途。
